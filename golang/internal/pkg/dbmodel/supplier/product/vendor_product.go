...
func AddNewVendorProduct(tx *sql.Tx, newProduct VendorNewProduct) (int, error) {
	...
	if newProduct.Price != nil {
		if newProduct.PriceCcyId == nil {
			return 0, fmt.Errorf("product %v of vendor %v has a price but missing currency information",
				newProduct.Reference,
				newProduct.VendorId)
		}
		if newProduct.PriceTypeId == nil {
			return 0, fmt.Errorf("product %v of vendor %v has a price but missing price type information",
				newProduct.Reference,
				newProduct.VendorId)
		}
