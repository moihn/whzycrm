...
func (handler *serviceHandlerImpl) GetClientProduct(w http.ResponseWriter, r *http.Request, clientId int, productReference string) {
	tx := handler.getDbTransaction(w)
	if tx == nil {
		return
	}
	...
	clientProducts, err := api.GetClientProducts(tx, clientId, productReference)
	if err != nil {
	...
	for _, clientProduct := range clientProducts {
		...
		if len(clientProduct.Description) > 0 {
			clientProductResp.Description = &clientProduct.Description
		}
		if len(clientProduct.Narrative) > 0 {
			clientProductResp.Narrative = &clientProduct.Narrative
		}
		if len(clientProduct.Barcode) > 0 {
			clientProductResp.Barcode = &clientProduct.Barcode
		}
...
func (handler *serviceHandlerImpl) AddVendorProduct(w http.ResponseWriter, r *http.Request) {
	var addVendorProductsBody AddVendorProductJSONRequestBody
	status := util.DecodeJsonBodyAsObject(w, r, handler.bodySizeLimit, &addVendorProductsBody)
	if status != nil {
		sendResponse(w, status)
		return
	}
	...
	newVendorProductSpecs := []api.VendorNewProduct{}
	for _, newProduct := range addVendorProductsBody.Products {
		description := ""
		...
		newVendorProductSpecs = append(newVendorProductSpecs, api.VendorNewProduct{
	...
	newVendorProductIds, err := api.AddVendorProducts(tx, newVendorProductSpecs)
	if err != nil {
	...
	response := AddVendorProductResponse{
		ProductIds: &newVendorProductIds,
	}
