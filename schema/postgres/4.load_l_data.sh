...
drop table if exists ${TMP_SCHEMA}.lr_catalog;
create table if not exists ${TMP_SCHEMA}.lr_catalog
(
Reference text,
Description_1 text,
Unit_L numeric,
...
${PSQL_CMD} -U ${DB_NAME}_appuser -d ${DB_NAME} -c "COPY ${TMP_SCHEMA}.lr_catalog(Reference,Description_1,Unit_L,Unit_W,Unit_H,Test_Peformed,Materials,Price,Description_2,Price_Type,MOQ,Qty_Per_Carton,Carton_L,Carton_W,Carton_H) FROM STDIN WITH (FORMAT CSV, HEADER true);" <${SCRIPT_DIR}/lr_spec.csv
...
begin
	...
	for spec in select * from ${TMP_SCHEMA}.lr_catalog
	loop
