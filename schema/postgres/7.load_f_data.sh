...
drop table if exists ${TMP_SCHEMA}.f_catalog;
create table if not exists ${TMP_SCHEMA}.f_catalog
(
Reference text,
Description_1 text,
Unit_L numeric,
...
${PSQL_CMD} -U ${DB_NAME}_appuser -d ${DB_NAME} -c "COPY ${TMP_SCHEMA}.f_catalog(Reference,Unit_L,Unit_W,Unit_H,Carton_L,Carton_W,Carton_H,Qty_Per_Carton,Description_1,Price,Inner_Qty,Materials,Price_Type,MOQ) FROM STDIN WITH (FORMAT CSV, HEADER true);" <${SCRIPT_DIR}/f_spec.csv
...
begin
	...
	for spec in select * from ${TMP_SCHEMA}.f_catalog
	loop
		...
		NULL, NULL, spec.qty_per_carton, spec.inner_qty,
		NULL, '2023-01-01');
